...
func init()  {
	fmt.Println("启动了msyql数据库")
	//fmt.Println("init函数")
	//拼接连接字符串
	connString:=conf.MysqlName+":"+conf.MysqlPwd+"@tcp("+conf.MysqlAddr+")/"+conf.MysqlDB+"?parseTime=true"
	...
	//连接池设置
	db.DB().SetMaxIdleConns(50)   //开始连接的空闲预备连接数据量
	db.DB().SetMaxOpenConns(100)   //最大连接数据量
	db.DB().SetConnMaxLifetime(60*30) //每个连接的空闲时间最大
